    ...
    <div className="min-h-[100dvh] bg-gray-50 flex flex-col font-sans text-slate-800 pb-[68px] md:pb-0">
      ...
      <header className="sticky top-0 z-[100] w-full bg-[#0D1F3C] shadow-md transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[68px] flex items-center justify-between">
          ...
          {/* Brand */}
          <Link href="/artisan" className="flex items-center gap-3 shrink-0 group">
            ...
            <div>
              ...
              <p className="text-blue-200 font-medium text-[10px] uppercase tracking-wider mt-0.5">Artisan Portal</p>
            </div>
          </Link>

          {/* Nav */}
          <AppNavigation role="ARTISAN" />
        </div>
      </header>
      ...
      <footer className="hidden md:block w-full py-8 text-center bg-white border-t border-gray-200 mt-auto">
        <p className="text-xs text-gray-400 font-medium">
          © {new Date().getFullYear()} Redeemer's University — Artisan Portal
        </p>
      </footer>
    </div>
  )
